...
const RiskDisclosure = memo(() => {
  ...
  const disclosuresCount = fields.length;
    ...
    try {
      ...
      if (isValid) {
        ...
        if (isEdit) {
          ...
          const res = await createRiskDisclosure({ ...values, assetId: id });
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-100 shadow-sm overflow-hidden relative"
      >
        {/* Background Pattern */}
        <div className="absolute inset-0 opacity-5">
          <div className="absolute top-4 right-4 w-32 h-32 bg-purple-500 rounded-full blur-3xl" />
          <div className="absolute bottom-4 left-4 w-24 h-24 bg-pink-500 rounded-full blur-2xl" />
        </div>
        ...
        <div className="relative">
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm font-medium mt-1">
                  Legal disclosures and compliance documentation for risk management
                </p>
              </div>
            </div>
            ...
            <motion.button
              onClick={handleAdd}
              className="bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white px-6 py-2 rounded-xl transition-all duration-300 flex items-center gap-2 shadow-lg hover:shadow-xl"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <Plus className="w-4 h-4" />
              <span className="font-medium">Add Disclosure</span>
            </motion.button>
          </div>
        </div>
      </motion.div>
      ...
      {disclosuresCount > 0 && (
        ...
        >
          ...
          <StatsCard
            ...
            value={disclosuresCount > 0 ? 'Active' : 'Pending'}
            color="pink"
          />
          <StatsCard
            icon={Archive}
            label="Documentation"
            value={disclosuresCount > 0 ? 'Complete' : 'Incomplete'}
            color="violet"
          />
        </motion.div>
      )}
        ...
        <Suspense fallback={
          <div className="flex items-center justify-center p-12">
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-gray-600 font-medium">Loading risk disclosures...</span>
            </div>
          </div>
        }>
          <div className="p-6">
            <VerticleTable
              ...
              emptyStateMessage={`No Risk Disclosures found`}
            />
          </div>
        </Suspense>
      </motion.div>
              ...
              >
                <DialogHeader>
                  ...
                  <p className="text-sm text-gray-600 mt-2">
                    {isEdit ? "Update disclosure information" : "Add a new risk disclosure with name and detailed description"}
                  </p>
                </DialogHeader>
                <div className="space-y-6 py-4">
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
                    ...
                    >
                      ...
                      {isLoading ? 'Saving...' : 'Save Disclosure'}
                    </Button>
                  </div>
                </div>
              </motion.div>
                ...
                <div className="space-y-4 py-4">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <p className="text-gray-700">
                      Are you sure you want to delete this Risk Disclosure? This action cannot be undone.
                    </p>
                  </div>
